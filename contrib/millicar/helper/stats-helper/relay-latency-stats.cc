...
namespace ns3 {
namespace mmwave {
RelayLatencyStats::RelayLatencyStats ()
{
}

void
RelayLatencyStats::SetDb (SQLiteOutput *db, const std::string & tableName)
{
  m_db = db;
  ...
  RelayLatencyStats::DeleteWhere (m_db, RngSeedManager::GetSeed (),
...
void
RelayLatencyStats::SaveRelayLatency (uint16_t sourceRnti, //uint16_t intermediateRnti, 
...
void
RelayLatencyStats::DeleteWhere (SQLiteOutput *p, uint32_t seed,
...
void RelayLatencyStats::WriteCache ()
{
  bool ret = m_db->SpinExec ("BEGIN TRANSACTION;");
  for (const auto & v : m_relayLatencyCache)
    {
//	  NS_LOG_UNCOND("Writing cache " << v.cellId << " " << v.rnti<< " " << v.power<< " " << v.avgSinr << " " << v.bwpId);
    ...
    //   ret = m_db->Bind (stmt, 3, static_cast<uint32_t>(v.intermediateRnti));
      NS_ASSERT (ret);
      ret = m_db->Bind (stmt, 3, static_cast<uint32_t>(v.localRnti));
    //   NS_ASSERT (ret);
    //   ret = m_db->Bind (stmt, 5, static_cast<uint32_t>(v.position.x));
    //   NS_ASSERT (ret);
    //   ret = m_db->Bind (stmt, 6, static_cast<uint32_t>(v.position.y));
      NS_ASSERT (ret);
      ret = m_db->Bind (stmt, 4, static_cast<uint32_t>(v.packetId));
      NS_ASSERT (ret);
      ret = m_db->Bind (stmt, 5, v.txTime);
    //   NS_ASSERT (ret);
    //   ret = m_db->Bind (stmt, 9, v.seqNumber);
      NS_ASSERT (ret);
      ret = m_db->Bind (stmt, 6, v.packetSize);
      NS_ASSERT (ret);
      ret = m_db->Bind (stmt, 7, static_cast<uint32_t>(v.traceSource));
      NS_ASSERT (ret);
      ret = m_db->Bind (stmt, 8, RngSeedManager::GetSeed ());
      NS_ASSERT (ret);
      ret = m_db->Bind (stmt, 9, static_cast<uint32_t> (RngSeedManager::GetRun ()));
      NS_ASSERT (ret);
      ret = m_db->Bind (stmt, 10, v.timeInstance.GetSeconds());
