...
MmWaveSidelinkSpectrumPhy::SetDevice (Ptr<NetDevice> d)
{
  ...
  // end orginal
  // modified
   NS_ABORT_MSG_IF(!((DynamicCast<MmWaveVehicularNetDevice>(d)) || (DynamicCast<MmWaveMillicarUeNetDevice>(d))),
    "The MmWaveSidelinkSpectrumPhy only works with MmWaveVehicularNetDevices or MmWaveMillicarUeNetDevice");
  // end modified
  m_device = d;
}
...
MmWaveSidelinkSpectrumPhy::ConfigureBeamforming (Ptr<NetDevice> dev)
{
  ...
  Ptr<MmWaveMillicarUeNetDevice> gVehicularDevice = DynamicCast<MmWaveMillicarUeNetDevice>(dev);
  if (vehicularDevice)
    {
...
void
MmWaveSidelinkSpectrumPhy::SetBeamformingModel (Ptr<mmwave::MmWaveBeamformingModel> beamformingModel)
{
  NS_LOG_FUNCTION (this);
...
MmWaveSidelinkSpectrumPhy::GetBeamformingModel ()
{
  // NS_LOG_FUNCTION (this);
