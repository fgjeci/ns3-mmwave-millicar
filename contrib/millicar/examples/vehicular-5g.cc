...
int main (int argc, char *argv[])
{
  ...
  Config::SetDefault ("ns3::MmWaveMillicarHelper::E2ModeNr", BooleanValue (true));
  ...
  // create the nodes
  NodeContainer ueNodes, ueNodesGroup1, ueNodesGroup2, ueNodesGroup3, ueNodesGroup4;
  ueNodes.Create (8);
  ...
  uemobility.Install (ueNodesGroup2);
  // other groups
  
  rho = 20;
  center_x = 90;
  center_y = 60;
  uePositionAllocGroup1->SetX(90);
  uePositionAllocGroup1->SetY(60);
  ...
  // source
  ueNodes.Get (0)->GetObject<MobilityModel> ()->SetPosition (Vector (48,30,1.5));
  // relay node
  ...
  ueNodes.Get (4)->GetObject<MobilityModel> ()->SetPosition (Vector (45, 50,  1.5));
...
//   Ipv4StaticRoutingHelper ipv4RoutingHelper;
  ...
  *_stream->GetStream () << "rxtime" << "\t" << "rnti" << "\t" << "size" 
  ...
  //   // _ind+4
  //   Ptr<OutputStreamWrapper> stream = asciiTraceHelper.CreateFileStream (tracesPath+"rx-n"  + std::to_string(ueNodes.Get(_ind)->GetId()) + ".txt");
  ...
  //   Ptr<OutputStreamWrapper> stream = asciiTraceHelper.CreateFileStream(tracesPath+"tx-n" + std::to_string(ueNodes.Get(_ind)->GetId()) + ".txt");
  //   efsStreamFiles.push_back(stream);
  //   // adding the header to the file
  //   Ptr<mmwave::MmWaveMillicarUeNetDevice> ueNrUeNetDev = ueNetDev.Get(_ind)->GetObject<mmwave::MmWaveMillicarUeNetDevice>();
  //   // Ptr<millicar::MmWaveSidelinkPhy> uePhy = ueNrUeNetDev->GetPhyMillicar();
  //   // uePhy->SetAttribute("TxPower", DoubleValue (50.0));
  ...
  //   // OnOffHelper clientHelper(TcpSocketFactory::GetTypeId().GetName(), InetSocketAddress(ueNodesIpv4InterfaceContainer.GetAddress(_ind+4), (otherPort + 2 * (_ind+4))));
  //   // clientHelper.SetAttribute("OnTime", StringValue("ns3::ConstantRandomVariable[Constant=0.11]"));
  //   // clientHelper.SetAttribute("OffTime", StringValue("ns3::ConstantRandomVariable[Constant=0]"));
  //   // clientHelper.SetAttribute("DataRate", StringValue("10Mb/s"));
  //   // clientHelper.SetAttribute("EnableSeqTsSizeHeader", BooleanValue(true));
  //   // ApplicationContainer bulkSenderApplicationContainer = clientHelper.Install(ueNodes.Get(_ind));
  //   // Ptr<OnOffApplication> bulkSenderApplication = bulkSenderApplicationContainer.Get(0)->GetObject<OnOffApplication>();
  ...
  //    << "remaining" << "\t" << "sent" << "\t" << "availTx" << "\t" << "time" << std::endl;
  ...
  // for (int _ind = 0; _ind < 1; ++_ind){
    ...
    // udpEchoServerApp->TraceConnectWithoutContext ("Rx", MakeBoundCallback (&Rx, _stream));
  ...
  *stream->GetStream () << "rnti" << "\t" << "imsi" << "\t" << "nodeid" << "\t"  << "cellid" << "\t" 
  ...
  // for (int _ind = 0; _ind < 1; ++_ind){
    ...
    bulkSenderStruct->stream = stream;
    bulkSenderStruct->pointerToSize = &efsSize.at(_ind);
    // udpEchoClientApp->TraceConnectWithoutContext("Tx", 
    //     MakeBoundCallback(&mmwave::ParametersConfig::BulkApplicationTxPacketLteCoordinator, 
    //     bulkSenderStruct , DynamicCast<LteEnbNetDevice>(lteEnbNodes.Get(0))));
  ...
  // for (int _ind = 0; _ind < 0; ++_ind){
    ...
    // // echoApps.Start (Seconds (0.0));
    // // Ptr<OutputStreamWrapper> _stream = asciiTraceHelper.CreateFileStream ("simple-one-stats.txt");
    // echoApp.Get(0)->TraceConnectWithoutContext ("Rx", MakeBoundCallback (&Rx, _stream));
    udpEchoServerApp->TraceConnectWithoutContext ("RxWithAddresses", 
  ...
  //   UdpEchoServerHelper server (port);
  //   ApplicationContainer echoApp = server.Install (ueNodesGroup2.Get (_ind+4));
  //   echoApps.Add(echoApp);
  //   // // echoApps.Start (Seconds (0.0));
  //   // // Ptr<OutputStreamWrapper> _stream = asciiTraceHelper.CreateFileStream ("simple-one-stats.txt");
  //   echoApp.Get(0)->TraceConnectWithoutContext ("Rx", MakeBoundCallback (&Rx, _stream));
