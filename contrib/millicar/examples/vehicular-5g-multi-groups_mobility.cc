...
{   
  ...
  // create the nodes
  NodeContainer ueNodesRandom, ueNodesGroup1Random, ueNodesGroup2Random, ueNodesGroup3Random, ueNodesGroup4Random, ueNodesGroup5Random;
  ...
  relayNodes.Add(ueNodes.Get (1));
  relayNodes.Add(ueNodesGroup1.Get (1));
  relayNodes.Add(ueNodesGroup2.Get (1));
  relayNodes.Add(ueNodesGroup3.Get (1));
  relayNodes.Add(ueNodesGroup4.Get (1));
  relayNodes.Add(ueNodesGroup5.Get (1));
...
//   Ipv4StaticRoutingHelper ipv4RoutingHelper;
  ...
  *stream->GetStream () << "rnti" << "\t" << "imsi" << "\t" << "nodeid" << "\t"  << "cellid" << "\t" 
  ...
  // for (int _ind = 0; _ind < 1; ++_ind){
    ...
    ApplicationContainer echoApp = videoServer.Install (ueNodes.Get (_ind));
    Ptr<VideoStreamServer> udpEchoServerApp = echoApp.Get(0)->GetObject<VideoStreamServer>();
    packetSinkApps.Add(echoApp);
    udpEchoServerApp->TraceConnectWithoutContext ("RxWithAddresses", 
  ...
  int trafficGeneratingNodes = 0;

  if(activeGroups==2){
    trafficGeneratingNodes = 2;
  }else{
    trafficGeneratingNodes = 0;
  }
  ...
  for (int _ind = 0; _ind < trafficGeneratingNodes; ++_ind){
    ...
    VideoStreamClientHelper videoClient (ueNodes.Get (_ind+4)->GetObject<Ipv4> ()->GetAddress (1, 0).GetLocal (), port);
    ApplicationContainer udpApp = videoClient.Install (ueNodes.Get (_ind));
    Ptr<VideoStreamClient> udpEchoClientApp = udpApp.Get(0)->GetObject<VideoStreamClient>();
    bulkApps.Add(udpApp);
  }
  ...
  // for (int _ind = 0; _ind < 0; ++_ind){
    ...
    videoServer.SetAttribute ("MaxPacketSize", UintegerValue (1400));
    videoServer.SetAttribute ("FrameFile", StringValue ("./scratch/videoStreamer/frameList.txt"));
    ApplicationContainer echoApp = videoServer.Install (ueNodesGroup1.Get (_ind));
    Ptr<VideoStreamServer> udpEchoServerApp = echoApp.Get(0)->GetObject<VideoStreamServer>();
    echoApps.Add(echoApp);
    udpEchoServerApp->TraceConnectWithoutContext ("RxWithAddresses", 
  ...
  // for (int _ind = 0; _ind < ueNodesGroup1.GetN()/2; ++_ind){
    ...
    ApplicationContainer udpApp = videoClient.Install (ueNodesGroup1.Get (_ind));
    Ptr<VideoStreamClient> udpEchoClientApp = udpApp.Get(0)->GetObject<VideoStreamClient>();
    updApps.Add(udpApp);
  }
  ...
  // for (int _ind = 0; _ind < 0; ++_ind){
    ...
    videoServer.SetAttribute ("MaxPacketSize", UintegerValue (1400));
    videoServer.SetAttribute ("FrameFile", StringValue ("./scratch/videoStreamer/frameList.txt"));
    ApplicationContainer echoApp = videoServer.Install (ueNodesGroup2.Get (_ind));
    Ptr<VideoStreamServer> udpEchoServerApp = echoApp.Get(0)->GetObject<VideoStreamServer>();
    echoApps.Add(echoApp);
    udpEchoServerApp->TraceConnectWithoutContext ("RxWithAddresses", 
  ...
  // for (int _ind = 0; _ind < ueNodesGroup2.GetN()/2; ++_ind){
    ...
    ApplicationContainer udpApp = videoClient.Install (ueNodesGroup2.Get (_ind));
    Ptr<VideoStreamClient> udpEchoClientApp = udpApp.Get(0)->GetObject<VideoStreamClient>();
    updApps.Add(udpApp);
  }
  ...
  // for (int _ind = 0; _ind < 0; ++_ind){
    ...
    videoServer.SetAttribute ("MaxPacketSize", UintegerValue (1400));
    videoServer.SetAttribute ("FrameFile", StringValue ("./scratch/videoStreamer/frameList.txt"));
    ApplicationContainer echoApp = videoServer.Install (ueNodesGroup3.Get (_ind));
    Ptr<VideoStreamServer> udpEchoServerApp = echoApp.Get(0)->GetObject<VideoStreamServer>();
    echoApps.Add(echoApp);
    udpEchoServerApp->TraceConnectWithoutContext ("RxWithAddresses", 
  ...
  // for (int _ind = 0; _ind < ueNodesGroup3.GetN()/2; ++_ind){
    ...
    ApplicationContainer udpApp = videoClient.Install (ueNodesGroup3.Get (_ind));
    Ptr<VideoStreamClient> udpEchoClientApp = udpApp.Get(0)->GetObject<VideoStreamClient>();
    updApps.Add(udpApp);
  }
  ...
  for (auto ueDeviceIt = allUeDevices.Begin();ueDeviceIt != allUeDevices.End(); ++ueDeviceIt){
    ...
    ueMac->TraceConnectWithoutContext("RlcBufferStatus",
											   MakeBoundCallback(&mmwave::EfStatsHelper::ReportMacBsr, &macBsrStats));
