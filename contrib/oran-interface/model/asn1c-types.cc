...
// ServingCellMeasurementsWrap::ServingCellMeasurementsWrap (Ptr<L3RrcMeasurements> l3RrcMeasurements)
// {

  // m_servingCellMeasurements =
  //     (ServingCellMeasurements_t *) calloc (1, sizeof (ServingCellMeasurements_t));
  // m_servingCellMeasurements->present = ServingCellMeasurements_PR_nr_measResultServingMOList;
  
  // m_servingCellMeasurements =servingCellMeas->GetPointer();
  // m_nr_measResultServingMOList =
  //         (MeasResultServMOList_t *) calloc (1, sizeof (MeasResultServMOList_t));
  
  // m_servingCellMeasurements = l3RrcMeasurements->GetPointer()->servingCellMeasurements;
  // m_nr_measResultServingMOList = l3RrcMeasurements->GetPointer()->servingCellMeasurements->choice.nr_measResultServingMOList;
// }
...
// modified
void
MeasQuantityResultsWrap::AddMcs (uint8_t mcs){
  m_measQuantityResults->mcs = (MCS_t *) calloc (1, sizeof (MCS_t));
  *m_measQuantityResults->mcs = (long)mcs;
}
// end modification
...
Ptr<L3RrcMeasurements>
L3RrcMeasurements::CreateL3RrcUeSpecificSinrMcsServing (long servingCellId, long physCellId, long sinr, uint8_t mcs)
{
  Ptr<L3RrcMeasurements> l3RrcMeasurement = Create<L3RrcMeasurements> (RRCEvent_b1);
  Ptr<ServingCellMeasurementsWrap> servingCellMeasurements =
      Create<ServingCellMeasurementsWrap> (ServingCellMeasurements_PR_nr_measResultServingMOList);
  ...
  Ptr<MeasQuantityResultsWrap> measQuantityResultWrap = Create<MeasQuantityResultsWrap> ();
  measQuantityResultWrap->AddSinr (sinr);
  measQuantityResultWrap->AddMcs(mcs);
  measResultNr->AddCellResults (MeasResultNr::SSB, measQuantityResultWrap->GetPointer ());
  Ptr<MeasResultServMo> measResultServMo =
      Create<MeasResultServMo> (servingCellId, measResultNr->GetValue ());
  servingCellMeasurements->AddMeasResultServMo (measResultServMo->GetPointer ());

  // Ptr<MeasResultNr> measResultNr1 = Create<MeasResultNr> (physCellId);
  // Ptr<MeasQuantityResultsWrap> measQuantityResultWrap1 = Create<MeasQuantityResultsWrap> ();
  // measQuantityResultWrap1->AddSinr (sinr);
  // measQuantityResultWrap1->AddMcs(mcs);
  // measResultNr1->AddCellResults (MeasResultNr::SSB, measQuantityResultWrap1->GetPointer ());
  // Ptr<MeasResultServMo> measResultServMo1 =
  //     Create<MeasResultServMo> (servingCellId, measResultNr1->GetValue ());
  // servingCellMeasurements->AddMeasResultServMo (measResultServMo1->GetPointer ());
...
// modified
void
L3RrcMeasurements::AddActivePeerDeviceSinrMcs(Ptr<ServingCellMeasurementsWrap> servingCellMeasurements, long peerDeviceId, long sinr, uint8_t mcs){ 
  // Ptr<ServingCellMeasurementsWrap> servingCellMeasurements =
  //     Create<ServingCellMeasurementsWrap> (Ptr<L3RrcMeasurements>(this));
  // Ptr<ServingCellMeasurementsWrap> servingCellMeasurements =
  //   Create<ServingCellMeasurementsWrap> (ServingCellMeasurements_PR_nr_measResultServingMOList);  
  // Ptr<ServingCellMeasurements> servingCellMeasurements = m_l3RrcMeasurements->servingCellMeasurements

  Ptr<MeasResultNr> measResultNr = Create<MeasResultNr> (peerDeviceId);
  Ptr<MeasQuantityResultsWrap> measQuantityResultWrap = Create<MeasQuantityResultsWrap> ();
  measQuantityResultWrap->AddSinr (sinr);
  measQuantityResultWrap->AddMcs(mcs);
  measResultNr->AddCellResults (MeasResultNr::SSB, measQuantityResultWrap->GetPointer ());
  Ptr<MeasResultServMo> measResultServMo =
      Create<MeasResultServMo> (peerDeviceId, measResultNr->GetValue ());
  servingCellMeasurements->AddMeasResultServMo (measResultServMo->GetPointer ());

  // this->AddServingCellMeasurement (servingCellMeasurements->GetPointer ());

  // Ptr<MeasResultNr> measResultNr1 = Create<MeasResultNr> (peerDeviceId);
  // Ptr<MeasQuantityResultsWrap> measQuantityResultWrap1 = Create<MeasQuantityResultsWrap> ();
  // measQuantityResultWrap1->AddSinr (sinr);
  // measQuantityResultWrap1->AddMcs(mcs);
  // measResultNr1->AddCellResults (MeasResultNr::SSB, measQuantityResultWrap1->GetPointer ());
  // Ptr<MeasResultServMo> measResultServMo1 =
  //     Create<MeasResultServMo> (peerDeviceId, measResultNr1->GetValue ());
  // servingCellMeasurements->AddMeasResultServMo (measResultServMo1->GetPointer ());
}

Ptr<MeasResultServMo>
L3RrcMeasurements::CreateActivePeerDeviceSinrMcs(long peerDeviceId, long sinr, uint8_t mcs){ 
  ...
  return Create<MeasResultServMo> (peerDeviceId, measResultNr->GetValue ());
}


Ptr<L3RrcMeasurements>
L3RrcMeasurements::CreateL3RrcUeSpecificPeerDevicesMeasurements ()
{
  // Ptr<L3RrcMeasurements> l3Measurements = Create<L3RrcMeasurements> (RRCEvent_b1);
  // Ptr<ServingCellMeasurementsWrap> servingCellMeasurements =
  //     Create<ServingCellMeasurementsWrap> (ServingCellMeasurements_PR_nr_measResultServingMOList); 
  // l3Measurements->AddServingCellMeasurement (servingCellMeasurements->GetPointer ());
  // return l3Measurements;
...
void
L3RrcMeasurements::AddNeighbourCellMeasurementMcs (long neighCellId, long sinr, uint8_t mcs)
{
