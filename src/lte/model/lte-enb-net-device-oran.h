...
#ifndef LTE_ENB_NET_DEVICE_ORAN_H 
#define LTE_ENB_NET_DEVICE_ORAN_H
 ...
 */
class LteEnbNetDeviceOran : public LteNetDevice
{
public:
const static uint16_t E2SM_REPORT_MAX_NEIGH = 8;
  ...
  virtual ~LteEnbNetDeviceOran (void);
   ...
   * \sa LteEnbNetDeviceOran::SetCsgId
   */
   ...
   *
   * \sa LteEnbNetDeviceOran::SetCsgIndication
   */
  ...
  struct SinrMcsPair
    {
        long double sinr; ///< RNTI
        uint8_t mcs = 0; ///< DRBID

        SinrMcsPair(long double sinr, uint8_t mcs)
        : sinr(sinr), mcs(mcs)
        {
        }
    ...
    static void RegisterSlSinrReportReadingCallback(Ptr<LteEnbNetDeviceOran> netDev, std::string context, uint16_t localRnti, uint16_t destRnti, uint8_t numSym, uint32_t tbSize, double avgSinr);

    void RegisterSlSinrReportReading(uint16_t localRnti, uint16_t destRnti, uint8_t numSym, uint32_t tbSize, double avgSinr);

    std::map<uint16_t, std::map<uint16_t, SinrMcsPair>> m_l3sinrMap;
...
private:
   ...
   * the LteEnbNetDeviceOran from a user perspective, but are actually used also in
...
}; // end of class LteEnbNetDeviceOran

} // namespace ns3

#endif /* LTE_ENB_NET_DEVICE_ORAN_H */
